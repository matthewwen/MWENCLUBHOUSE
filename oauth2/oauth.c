...
unsigned char KEY256[] = "f1a66c30049324c6fdd4f51c33f8364b9be07c4f4c7bc0bf10d0e782eb2f099b";

#define TABLE_NAME   "usertable"
#define CREATE_TABLE "CREATE TABLE IF NOT EXISTS "TABLE_NAME"(id INTEGER PRIMARY KEY, username TEXT, hashedpwd TEXT, admin INTEGER);"
...
char * oauth_get_hashed_pwd(const char * username, char ** a_error_msg) {
	sqlite3 * db = NULL;
	char * error_msg = NULL;
	const char * select_format = "SELECT (hashedpwd) FROM "TABLE_NAME" WHERE username=\"%s\";";
	char * output = NULL;
...
void oauth_signup(char * username, char * pwd) {
	sqlite3 * db = NULL;
	char * error_msg = NULL;
	char * insert_format = "INSERT INTO "TABLE_NAME"(username, hashedpwd, admin) VALUES (\"%s\", \"%s\", 0);";
...
bool oauth_isValid_token(char * token, oauth_sec_t req_security) {
	...
	const char * select_format = "SELECT (admin) FROM "TABLE_NAME" WHERE username=\"%s\";";
...
void oauth_update_security(char * token, char * username, oauth_sec_t sec) {
	...
	const char * select_format = "UPDATE "TABLE_NAME" SET admin=%ld WHERE username=\"%s\";";
	int len = 0;
...
void oauth_init_admin() {
	...
	char * update_format = "UPDATE "TABLE_NAME" SET admin=%ld WHERE username=\"mwen\";";
